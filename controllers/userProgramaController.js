...
const { getBotResponse, generatePrograms } = require("./openaiController");
...
exports.createAndGenerateReport = async (req, res) => {
  ...
  try {
    ...
    if (!estresSession || !userResponse || !user) {
      return res
        .status(404)
        .json({ error: "No se encontraron los datos requeridos del usuario." });
    }

    const estres_nivel =
      estresNivelMap[estresSession.estres_nivel_id] || "Desconocido";
    const age_range = ageRangeMap[userResponse.age_range_id] || "Desconocido";
    const hierarchical_level =
      hierarchicalLevelMap[userResponse.hierarchical_level_id] || "Desconocido";
    const responsability_level =
      responsabilityLevelMap[userResponse.responsability_level_id] ||
      "Desconocido";
    ...
    console.log(
      "Todos los Registros insertados correctamente en la tabla UserPrograma"
    );
...
exports.getByUserId = async (req, res) => {
  ...
  try {
    ...
    const { count, rows: userProgramas } = await UserPrograma.findAndCountAll({
      ...
      include: [
        {
          model: EstresTecnicas,
          as: "tecnica",
          attributes: ["id", "nombre", "mensaje", "steps", "tipo", "icon"],
        },
      ],
    });
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    userPrograma.estrestecnicas_id =
      estrestecnicas_id || userPrograma.estrestecnicas_id;
